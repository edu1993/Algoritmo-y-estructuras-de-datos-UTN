//Apellido, Nombre nacido el XX\XX\XXXX con XX de edad.
#include <iostream>
#include <string.h>
#include <ctype.h>
struct nacf
...
int main()
{
	...
	do
	{
		...
		for (int i=0;i <=2;i++)
		{
			printf("%s, %s nacido el %d\%d\%d con %d de edad",persona[i].apellido,persona[i].nombre,persona[i].nac.dia,persona[i].nac.mes,persona[i].nac.ano,persona[i].edad);
			printf("\n \n");
		}
